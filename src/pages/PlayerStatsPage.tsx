  ...
  return (
    <TenantRequired>
      <div className="space-y-6">
        <PageHeader title="Player statistics" backTo={`/dashboard/players/${id}`} backLabel="Back to player" />
        {isLoading && !data ? (
          <LoadingSpinner className="h-8 w-8 text-[#12233D]" />
        ) : isError || !data ? (
          <div className="rounded-lg border border-red-200 bg-red-50 p-6 text-red-700">Unable to load player stats.</div>
        ) : (
          <div className="grid max-w-2xl gap-4 rounded-lg border bg-white p-6 sm:grid-cols-2">
            ...
            <Link to={`/dashboard/players/${id}`} className="text-sm font-medium text-[#12233D] underline sm:col-span-2">
              Edit player profile
            </Link>
          </div>
        )}
