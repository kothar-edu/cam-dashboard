...
function TeamLabel({ team, muted }: { team: LiveOpponent | null | undefined; muted?: boolean }) {
  return (
    <span
      className={`flex shrink-0 items-center gap-1.5 ${muted ? 'font-semibold text-slate-500' : 'font-extrabold text-[#12233D]'}`}
    >
      {team?.logo && (
        <img
          src={team.logo}
...
export function CommentaryBar({
  ...
  useEffect(() => {
    const timer = setTimeout(
      ...
      showStats ? STATS_DURATION_MS : SPONSOR_DURATION_MS
    );
    ...
    <div className="absolute inset-x-0 bottom-0 flex h-10 w-full items-center justify-center gap-4 bg-blue-950 text-xl font-bold leading-none">
      ...
      <div className="flex h-full grow items-center justify-center gap-3 overflow-hidden px-2">
        {(showChase || showProjected) && (
          <div
            key="stats"
            className="flex animate-in fade-in slide-in-from-bottom-2 items-center gap-3 duration-500"
          >
            {showChase && (
              <>
        ...
        {showSponsorSlot && (
          <div
            key="sponsor"
            className="max-w-[700px] animate-in fade-in slide-in-from-bottom-2 duration-500"
          >
